...
function RentListPage({ user }) {
  ...
  const fetchRentRecords = async () => {
    ...
    try {
      ...
      const rentRecords = response.data.filter(record => record.type === 'Rent');
      setRecords(rentRecords);
      setError('');
    } catch (err) {
      setError('Failed to fetch rent records: ' + (err.response?.data || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRowClick = (id, name) => {
    navigate(`/rent/${id}/${name.replace(/ /g, '-')}`);;
  };
  ...
  return (
    <Box className="record-container">
      ...
      {records.length === 0 && !loading && (
        <Typography sx={{ mt: 2, textAlign: 'center', color: '#8B0000' }}>No rent records</Typography>
      )}
      {records.length > 0 && (
